...
import { UpdateSeenOfDoc } from "../../lib/firebase-helpers"
...
const NotificationCard = () => {
  ...
  const targetParagraph = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    observer.observe(targetParagraph?.current as Element);
  ...
  useEffect(()=>{
    const arr = data?.headerNotification.filter(it => (!it.seen)).map(it => it.uniqueId)!;
    if(isIntersecting){
      UpdateSeenOfDoc({ids: arr})
    }
  }, [isIntersecting])
  return(
    <div ref={targetParagraph} className="divide-y flex flex-col gap-2">
        ...
        <div>
            <HoverCardDemo text="Notifications">
                ...
                >
                    <Bell />
                    {
                    notification?.numberOfNotifications! > 0 && <span className="absolute top-0 right-1.5">{notification?.numberOfNotifications}</span>
                    }
                </Button>
            </HoverCardDemo>
